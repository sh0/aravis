...
arv_gc_boolean_get_access_mode (ArvGcFeatureNode *gc_feature_node)
{
	ArvGcBoolean *gc_boolean = NULL;
	ArvGcNode *pvalue_node = NULL;
	ArvGcFeatureNode *pvalue_feature = NULL;

	g_return_val_if_fail (ARV_IS_GC_BOOLEAN (gc_feature_node), ARV_GC_ACCESS_MODE_RO);
	gc_boolean = ARV_GC_BOOLEAN (gc_feature_node);
	if (gc_boolean->value == NULL)
...
arv_gc_boolean_class_init (ArvGcBooleanClass *this_class)
{
	...
	gc_feature_node_class->get_access_mode = arv_gc_boolean_get_access_mode;
}
