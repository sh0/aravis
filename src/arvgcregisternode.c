...
{
	ArvGcRegisterNode *self = NULL;
	ArvGcRegisterNodePrivate *priv = NULL;

	g_return_val_if_fail (ARV_IS_GC_REGISTER_NODE (gc_feature_node), ARV_GC_ACCESS_MODE_RO);
	self = ARV_GC_REGISTER_NODE(gc_feature_node);

	priv = arv_gc_register_node_get_instance_private (ARV_GC_REGISTER_NODE (self));
